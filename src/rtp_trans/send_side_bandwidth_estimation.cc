...
namespace live_stream_sdk {
  ...
  // zhangle
  void SendSideBandwidthEstimation::OnReceivedRtcpReceiverReport(const std::list<avformat::RTCPReportBlock>& report_blocks,
    int64_t rtt,
    int64_t now_ms) {
    ...
    // blocks.
    for (std::list<avformat::RTCPReportBlock>::const_iterator it = report_blocks.begin();
    ...
    pthread_mutex_lock(&mutex_);
    UpdateReceiverBlock(fraction_lost_aggregate, rtt,
      total_number_of_packets, now_ms);
    pthread_mutex_unlock(&mutex_);
  }

  // zhangle
  void SendSideBandwidthEstimation::OnReceivedEstimatedBitrate(uint32_t bitrate) {
