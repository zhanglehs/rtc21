...
namespace live_stream_sdk
{
  ...
  std::string SdpInfo::load(std::string& sdp)
  {
    return load(sdp.c_str(), static_cast<uint32_t>(sdp.length()));
  }

  std::string SdpInfo::load(const char* sdp, uint32_t len)
  {
    if (sdp == NULL || len <= 0)
    {
      ERR("sdp is invalid");
  ...
  std::string SdpInfo::get_sdp_str() const
  {
    return _sdp_ss.str();
  }
  ...
  int SdpInfo::add_media(rtp_media_info *s)
  {
    ...
    return static_cast<int>(_media_infos.size());
  }
  ...
  int SdpInfo::set_receiver_ip(std::string& ip)
  {
    _sdp_header->dst_addr = ip;
    generate_sdp_str();
    return 0;
  }

  int SdpInfo::set_streamid(std::string& id)
  {
    _sdp_header->name = id;
    generate_sdp_str();
    return 0;
  ...
  const std::vector<rtp_media_info*>& SdpInfo::get_media_infos()
  {
    return _media_infos;
  }

  void SdpInfo::set_sdp(const std::vector<rtp_media_info*>& media_infos,
    const sdp_session_level *header /*= NULL*/) {
    if (header) {
      *_sdp_header = *header;
    }
    else {
      _sdp_header->sdp_version = 0;
      _sdp_header->id = 0;
      _sdp_header->version = 0;
      _sdp_header->start_time = 0;
      _sdp_header->end_time = 0;
      _sdp_header->ttl = 0;
      _sdp_header->user = "-";
      _sdp_header->src_addr = "127.0.0.1";
      _sdp_header->src_type = "IP4";
      _sdp_header->dst_addr = "127.0.0.1";
      _sdp_header->dst_type = "IP4";
      _sdp_header->name = "noname";
    }

    clear_media();

    for (auto it = media_infos.begin(); it != media_infos.end(); it++) {
      if (*it) {
        add_media(new rtp_media_info(**it));
      }
    }
  ...
  void SdpInfo::parse_sdp_str(const char *sdp_str, uint32_t len)
  {
    ...
    for (;;) {
      ...
      /* get the content */
      q = buf;
      while (*p != '\n' && *p != '\r' && *p != '\0' && (p - sdp_str) <= (int)len)
      {
        if ((q - buf) < (uint32_t)sizeof(buf) - 1)
          *q++ = *p;
        p++;
      }
      if ((p - sdp_str) > (int)len)
      {
    ...
    case 'a':
      if (strstart(p, "rtpmap:", &p) && _media_infos.size() > 0) {
        ...
        if (_media_infos.size() > 0) {
          ...
          get_word_sep(buf1, sizeof(buf1), "/ ", &p);
          if (stricmp(buf1, "H264") == 0)
          {
            md->payload_type = payload_type;
          }
          else if (stricmp(buf1, "mpeg4-generic") == 0)
          {
            md->payload_type = payload_type;
          }
          else
        ...
        if (s1->seen_fmtp)
        {
          ...
          {
            ret = parse_fmtp(md, s1->delayed_fmtp, this->parse_h264_sdp_line);
          }
          else if (RTP_AV_AAC == md->payload_type)
          {
            ret = parse_fmtp(md, s1->delayed_fmtp, this->parse_aac_sdp_line);
          }
        ...
        if (s1->seen_rtpmap && _media_infos.size() > 0)
        {
          ...
          if (RTP_AV_H264 == md->payload_type)
          {
            ret = parse_fmtp(md, p, parse_h264_sdp_line);
          }
          else if (RTP_AV_AAC == md->payload_type)
          {
            ret = parse_fmtp(md, p, parse_aac_sdp_line);
          }
        }
        else {
          s1->seen_fmtp = 1;
  ...
  std::string SdpInfo::generate_sdp_str()
  {
    //int len = 0;
    _sdp_ss.clear();
    _sdp_ss.str("");
    ...
    try
    {
      int ret = sdp_write_header(_sdp_ss, _sdp_header);
      if (ret < 0)
      {
        throw ret;
      }
      ...
      {
        ret = sdp_write_media(_sdp_ss, _media_infos[i]);
        if (ret < 0)
        {
          throw ret;
        }
  ...
  int SdpInfo::sdp_write_address(std::stringstream& ss, const char *dest_addr, const char *dest_type, int ttl)
  {
  ...
  int SdpInfo::sdp_write_header(std::stringstream& ss, sdp_session_level *s)
  {
    char buff[1024];
    ::sprintf(buff, "v=%d\r\n"
      "o=- %d %d IN %s %s\r\n"
    ...
    ret = sdp_write_address(ss, s->dst_addr.c_str(), s->dst_type.c_str(), s->ttl);
  ...
  int SdpInfo::sdp_write_media(std::stringstream& ss, rtp_media_info *s)
  {
    const char *type;
    ...
    ret = sdp_write_address(ss, s->dest_addr.c_str(), "IP4", _sdp_header->ttl);

    //if (s->rate) {
    //	sprintf_s(buff, size, "b=AS:%d\r\n", s->rate);
    //}

    ret = sdp_write_media_attributes(ss, s);
    if (ret < 0)
    {
      return ret;
    }
  ...
  int SdpInfo::sdp_write_media_attributes(std::stringstream& ss, rtp_media_info *s)
  {
    char config[1024];
    char buff[1024];
    config[0] = 0;
    ...
    case RTP_AV_H264:
      ...
      else
      {
        ...
        //    s->payload_type);
        //ss << buff;
        //ERR("can not find sps/pps");
        //return -1;
      }

      if (strlen(config) == 0)
      {
        //ERR("encode sps/pps error");
        //return -1;
      }
      ::sprintf(buff, "a=rtpmap:%d H264/90000\r\n", s->payload_type);
      ss << buff;
      ...
      if (strlen(config) > 0)
      {
        ::sprintf(buff, "a=fmtp:%d packetization-mode=%d%s\r\n",
          s->payload_type, 1, config);
        ss << buff;
      }
      break;
  ...
  char *SdpInfo::extradata2psets(rtp_media_info *s, char* psets)
  {
    char *p;
    const uint8_t *r;
    ...
    {
      WRN("Too long extradata! payload_type %d extra_len %d addr %s \n",
    ...
    while (r < extradata + extradata_size)
    {
      ...
      if (!base64_encode(p, 1024 - static_cast<int>((p - psets)), r, static_cast<int>(r1 - r)))
      {
  ...
  char *SdpInfo::extradata2config(rtp_media_info *s, char* config)
  {
    if (s->extra_data_len > 1024)
    {
    ...
    if (s->extra_data != NULL && s->extra_data_len > 0)
    {
      extra_data = s->extra_data;
      extra_data_len = s->extra_data_len;
    }
    else
    {
      aac_config.set_channelConfig(s->channels);
      aac_config.set_samplingFrequencyIndex(aac_get_samplingFrequencyIndex(s->rate));
      aac_config.set_audioObjectType(s->h264_profile_level_id);

      extra_data = aac_config.latm_header.header_data;
      extra_data_len = 2;
    }

    memcpy(config, "; config=", 9);
    data_to_hex(config + 9, extra_data, extra_data_len, 0);
    config[9 + extra_data_len * 2] = 0;
...
#define AV_BSWAP32C(x) (AV_BSWAP16C(x) << 16 | AV_BSWAP16C((x) >> 16))
  ...
  out1:
  out0 :
    return bits & 1 ? -1 : static_cast<int>(dst - out);
  }
    ...
    int(*parse_fmtp_func)(rtp_media_info *s, const char *attr, const char *value))
  {
    ...
    char value[1024];
    int res;
    int value_size = static_cast<int>(strlen(p)) + 1;
